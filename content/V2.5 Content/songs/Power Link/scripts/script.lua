...
function createPost()
	...
	makeSprite('drunkSpeed', '', 50, 1)
	makeSprite('tipsySpeed', '', 1, 1)
	setActorAngle(50,'drunkSpeed')
...
function songStart()
	...
	tweenShaderProperty('vignette', 'size', 0.25, crochet*0.001*16*t, 'linear')
...
function stepHit()
	...
	if curStep >= 8 and curStep < 1152 then
		
		if section == 7 and secStep > 8 then 
			
		else 
		
			if secStep == 0 or secStep == 3 or secStep == 6 or secStep == 10 or secStep == 13 then 
				setAndEaseBackTo('drunk', 'angle', 0.5*swap, crochet*0.001*2, 'circOut', 0)
				swap = swap * -1
			end
		end
	...
	if section == 7 then 
		if secStep == 8 then 
			if not opponentPlay then 
				tweenShaderProperty('mirror', 'angle', -360.0, crochet*0.001*8, 'circIn')
			end
			
		elseif secStep == 12 then 
			tweenShaderProperty('mirror', 'zoom', 0.5, crochet*0.001*4, 'circIn')
		end
	elseif section == 8 and secStep == 0 then 
		tweenShaderProperty('mirror', 'zoom', 1.0, crochet*0.001*4, 'circOut')
	end


	if section == 12 or section == 13 then 
		if curStep % 8 == 0 then 
			setAndEaseBackTo('noteSine', 'x', 40, crochet*0.001*4, 'circOut', 0)
			setAndEaseBackTo('global', 'x', -100, crochet*0.001*4, 'circOut', 0)
	...
	elseif section == 14 then 
		if curStep % 4 == 0 then 
			setAndEaseBackTo('noteSine', 'x', 60, crochet*0.001*2, 'circOut', 0)
			setAndEaseBackTo('global', 'x', -150, crochet*0.001*2, 'circOut', 0)
			setAndEaseBackTo('drunk', 'x', -1, crochet*0.001*2, 'circOut', 0)
	...
	elseif section == 15 then 
		if secStep == 0 or secStep == 2 then 
			setAndEaseBackTo('noteSine', 'x', 100, crochet*0.001, 'circOut', 0)
			setAndEaseBackTo('global', 'x', -200, crochet*0.001, 'circOut', 0)
			setAndEaseBackTo('drunk', 'x', -1, crochet*0.001, 'circOut', 0)
		elseif secStep == 1 or secStep == 3 then 
			setAndEaseBackTo('noteSine', 'x', -100, crochet*0.001, 'circOut', 0)
			setAndEaseBackTo('global', 'x', 200, crochet*0.001, 'circOut', 0)
			setAndEaseBackTo('drunk', 'x', 1, crochet*0.001, 'circOut', 0)
		end
		...
		if secStep == 0 then 
			if not middlescroll then 
				...
				tweenActorProperty('opponent', 'x', -250, crochet*0.001*16, 'cubeIn')
			end

			tweenActorProperty('opponent', 'angle', -400, crochet*0.001*16, 'cubeIn')

			tweenActorProperty('screenRot', 'x', -360, crochet*0.001*16, 'cubeOut')
		end
	end


	
	if section == 17 or section == 21 or section == 29 or section == 49 or section == 53 then 
		if secStep == 8 then 
			if not middlescroll then 
				tweenActorProperty('player', 'x', 250, crochet*0.001*8, 'circIn')
			end
			tweenActorProperty('player', 'angle', -400, crochet*0.001*8, 'circIn')
		end
	elseif section == 18 or section == 22 or section == 30 or section == 50 or section == 54 then 
		if secStep == 0 then 
			if not middlescroll then 
				tweenActorProperty('opponent', 'x', 320, crochet*0.001*8, 'circOut')
			end
			tweenActorProperty('opponent', 'angle', 0, crochet*0.001*8, 'circOut')
		end
	end

	
	if section == 19 or section == 23 or section == 27 or section == 47 or section == 51 then 
		if secStep == 8 then 
			if not middlescroll then 
				tweenActorProperty('opponent', 'x', -250, crochet*0.001*8, 'circIn')
			end
			tweenActorProperty('opponent', 'angle', -400, crochet*0.001*8, 'circIn')
		end
	elseif section == 20 or section == 24 or section == 28 or section == 48 or section == 52 then 
		if secStep == 0 then 
			if not middlescroll then 
				tweenActorProperty('player', 'x', -320, crochet*0.001*8, 'circOut')
			end
			tweenActorProperty('player', 'angle', 0, crochet*0.001*8, 'circOut')
		end
	end

	if (section == 24 or section == 30 or section == 55) and secStep == 8 then 
		if not middlescroll then 
			tweenActorProperty('player', 'x', 0, crochet*0.001*8, 'circIn')
			tweenActorProperty('opponent', 'x', 0, crochet*0.001*8, 'circIn')
		end
		tweenActorProperty('player', 'angle', 0, crochet*0.001*8, 'circIn')
		tweenActorProperty('opponent', 'angle', 0, crochet*0.001*8, 'circIn')
	end
	

	if section == 25 then 
		if secStep == 0 then 
			tweenActorProperty('playerRot', 'x', 30, crochet*0.001*2, 'cubeOut')
			tweenActorProperty('opponentRot', 'x', -30, crochet*0.001*2, 'cubeOut')
		elseif secStep == 2 then 
			tweenActorProperty('playerRot', 'x', 60, crochet*0.001*2, 'cubeOut')
			tweenActorProperty('opponentRot', 'x', -60, crochet*0.001*2, 'cubeOut')
		elseif secStep == 4 then 
			tweenActorProperty('playerRot', 'x', 90, crochet*0.001*2, 'cubeOut')
			tweenActorProperty('opponentRot', 'x', -90, crochet*0.001*2, 'cubeOut')
		elseif secStep == 6 then 
			tweenActorProperty('playerRot', 'x', 360, crochet*0.001*8, 'expoOut')
			tweenActorProperty('opponentRot', 'x', -360, crochet*0.001*8, 'expoOut')
		end
	end

	if (section >= 16 and section < 32) or (section >= 48 and section < 56) then 
		if secStep == 0 or secStep == 10 then 
			setAndEaseBackTo('noteSine', 'angle', 300, crochet*0.001*4, 'cubeOut', 0)
		end
	end
	...
	if section == 27 then
		...
		if secStep == 8 then 
			tweenActorProperty('global', 'y', downscrollDiff*-50, crochet*0.001, 'expoOut')
		elseif secStep == 9 then 
			tweenActorProperty('global', 'y', downscrollDiff*-100, crochet*0.001, 'expoOut')
		elseif secStep == 10 then 
			tweenActorProperty('global', 'y', downscrollDiff*-150, crochet*0.001, 'expoOut')
		elseif secStep == 11 then 
			tweenActorProperty('global', 'y', downscrollDiff*-200, crochet*0.001, 'expoOut')
		elseif secStep == 12 then 
			tweenActorProperty('global', 'y', downscrollDiff*-250, crochet*0.001, 'expoOut')
		elseif secStep == 13 then 
			tweenActorProperty('global', 'y', downscrollDiff*-300, crochet*0.001, 'expoOut')
		elseif secStep == 14 then 
			tweenActorProperty('global', 'y', 0, crochet*0.001*2, 'expoOut')
		end
	...
	if section == 33 then 
		if secStep == 0 then 
			tweenActorProperty('playerRot', 'y', -90 + 22.5*downscrollDiff, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('playerIA', 'y', 22.5*downscrollDiff, crochet*0.001*4, 'cubeOut')
			
			tweenActorProperty('player', 'y', -520*0.125*downscrollDiff, crochet*0.001*4, 'cubeOut')
		elseif secStep == 6 then 
			tweenActorProperty('playerRot', 'y', -90 + 45*downscrollDiff, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('playerIA', 'y', 45*downscrollDiff, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('player', 'y', -520*0.25*downscrollDiff, crochet*0.001*4, 'cubeOut')
		end
	end

	if section == 35 then 
		if secStep == 0 then 
			tweenActorProperty('opponentRot', 'y', -90 - 22.5*downscrollDiff, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('opponentIA', 'y', -22.5*downscrollDiff, crochet*0.001*4, 'cubeOut')
			
			tweenActorProperty('opponent', 'y', -520*0.125*downscrollDiff, crochet*0.001*4, 'cubeOut')
		elseif secStep == 6 then 
			tweenActorProperty('opponentRot', 'y', -90 - 45*downscrollDiff, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('opponentIA', 'y', -45*downscrollDiff, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('opponent', 'y', -520*0.25*downscrollDiff, crochet*0.001*4, 'cubeOut')
		end
	end
	...
	if section == 37 then 
		if secStep == 0 then 
			...
			tweenActorProperty('player', 'y', -520*0.125*downscrollDiff, crochet*0.001*4, 'cubeOut')
		elseif secStep == 6 then 
			tweenActorProperty('opponentRot', 'y', -90, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('opponentIA', 'y', 0, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('opponent', 'y', 0, crochet*0.001*4, 'cubeOut')

			tweenActorProperty('playerRot', 'y', -90, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('playerIA', 'y', 0, crochet*0.001*4, 'cubeOut')
			tweenActorProperty('player', 'y', 0, crochet*0.001*4, 'cubeOut')
		end
	end

	if section >= 32 and curStep < 630 then 
		if section % 2 == 0 then 
			if secStep == 0 then 
				setAndEaseBackTo('noteSine', 'angle', 300, crochet*0.001*4, 'cubeOut', 0)
			end
	...
	if curStep >= 637 and curStep <= 760 then 

		if curStep % 8 == 0 then 
			setAndEaseBackTo('noteSine', 'angle', 300, crochet*0.001*4, 'cubeOut', 0)
			setAndEaseBackTo('tipsy', 'y', 1, crochet*0.001*4, 'cubeOut', 0)
			tweenActorProperty('global', 'angle', 0, crochet*0.001*2, 'expoOut')
		...
		elseif curStep % 8 == 2 then 
			tweenActorProperty('global', 'angle', -300, crochet*0.001*2, 'expoIn')
			tweenShaderProperty('mirror', 'angle', -10, crochet*0.001*2, 'cubeIn')
			tweenShaderProperty('mirror', 'zoom', 1.25, crochet*0.001*2, 'cubeIn')
		elseif curStep % 8 == 4 then 
			setAndEaseBackTo('noteSine', 'angle', 300, crochet*0.001*4, 'cubeOut', 0)
			setAndEaseBackTo('tipsy', 'y', -1, crochet*0.001*4, 'cubeOut', 0)
			tweenActorProperty('global', 'angle', 0, crochet*0.001*2, 'expoOut')
	...
	if section == 47 and secStep == 8 then 
		tweenActorProperty('playerRot', 'x', 0, crochet*0.001*8, 'cubeOut')
		tweenActorProperty('opponentRot', 'x', 0, crochet*0.001*8, 'cubeOut')
		tweenStageColorSwap('hue', 0, crochet*0.001*8, 'cubeIn')
	end

	if section == 60 and secStep == 0 then 

		if not middlescroll then
			tweenActorProperty('playerRot', 'x', 360*2, crochet*0.001*16*4, 'linear')
			tweenActorProperty('opponentRot', 'x', 360*2, crochet*0.001*16*4, 'linear')
			tweenActorProperty('screenRot', 'y', 360*2, crochet*0.001*16*4, 'linear')
		end
	elseif section == 64 and secStep == 0 then 

		if not middlescroll then
			tweenActorProperty('playerRot', 'x', 360*4, crochet*0.001*16*8, 'linear')
			tweenActorProperty('opponentRot', 'x', 360*4, crochet*0.001*16*8, 'linear')
			tweenActorProperty('screenRot', 'y', 360*4, crochet*0.001*16*8, 'linear')
		else 
			tweenActorProperty('playerRot', 'x', 360*2, crochet*0.001*16*8, 'linear')
			tweenActorProperty('opponentRot', 'x', 360*2, crochet*0.001*16*8, 'linear')
		end
		tweenActorProperty('global', 'y', downscrollDiff*-520, crochet*0.001*16*8, 'linear')
		tweenActorProperty('angle', 'angle', -360, crochet*0.001*16*8, 'linear')
		tweenActorProperty('global', 'alpha', 0, crochet*0.001*16*8, 'linear')
	end
