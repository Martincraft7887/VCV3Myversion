	...
	for (list in xml.elementsNamed("Init")) {
		for (event in list.elementsNamed("Event")) {
			switch(event.get("type")) {
				...
				case "initModifier":
					if (!legacyNoteModchart) continue;
					debugInitModifierCount++;
...
function applyLegacyEvent(currentStep:Float, e:Dynamic, skipImpulses:Bool):Bool {
	switch(e.type) {
		...
		case 4:
			if (!skipImpulses) camGame.zoom += e.value;
			return true;
		case 5:
			if (!skipImpulses) camHUD.zoom += e.value;
			return true;
	}
	return true;
}

function syncLegacyModchartToStep(step:Float, skipImpulses:Bool = true) {
	resetValuesToDefault();
...
function forceModchartSeekSync(step:Float = -1) {
	syncLegacyModchartToStep(step < 0 ? curStepFloat : step, true);
