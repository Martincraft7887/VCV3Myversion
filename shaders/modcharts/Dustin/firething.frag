...
// Sustituye noise1(), que existe en GLSL de escritorio pero no en todos los drivers.
float valueNoise1(in float value) {
    float cell = floor(value);
    float local = fract(value);
    local = local * local * (3.0 - 2.0 * local);
    float first = hash1_2(vec2(cell, 19.19));
    float second = hash1_2(vec2(cell + 1.0, 19.19));
    return mix(first, second, local) * 2.0 - 1.0;
}

vec2 noise2_2(in vec2 FUCK) {
    return vec2(valueNoise1(FUCK.x), valueNoise1(FUCK.y));
}

float noise1_2(in vec2 FUCK) {
    return (valueNoise1(FUCK.x) * valueNoise1(FUCK.y)) / 2.0;
}
...
float layeredNoise1_2(in vec2 uv, in float sizeMod, in float alphaMod, in int layers, in float animation)
{
    ...
    int safeLayers = layers;
    if (safeLayers < 1) safeLayers = 1;
    if (safeLayers > MAX_FIRE_LAYERS) safeLayers = MAX_FIRE_LAYERS;
    for (int i = 0; i < MAX_FIRE_LAYERS; i++)
    {
        if (i >= safeLayers) break;
    ...
    float normalization = abs(1.0 - alphaMod) < 0.0001
        ? 1.0 / float(safeLayers)
        : (1.0 - alphaMod) / max(1.0 - pow(alphaMod, float(safeLayers)), 0.0001);
    noise *= normalization;
    return noise;
}
...
{ 
    ...
    for (int i = 0; i < MAX_FIRE_LAYERS; i++)
    {
		...
		particles += fireParticles(bokehUV, uv) * alpha * (1.0 - smoothstep(0.0, 1.0, smoke) * (float(i) / float(safeLayers)));
...
void main()
{
    ...
    float effectAlpha = max(col.r, max(col.g, col.b));
    gl_FragColor = fcol + vec4(col, effectAlpha);
}
