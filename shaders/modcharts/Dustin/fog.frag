...
void main()
{
    ...
    float safeApplyRange = max(applyRange, 0.0001);
    float feather = max(float(FEATHER), 1.0);

    if (worldCoord.y <= applyY && worldCoord.y >= applyY - safeApplyRange) {
        float dist = applyY - worldCoord.y;
        float gradient = 1.0 - (dist / safeApplyRange);
