...
void main() {
	...
	if (boldSkewWidth) {
		float requestedPasses = clamp(floor((skewWidth / 8.0) + 0.5), 1.0, float(MAX_BOLD_PASSES));
		int passCount = int(requestedPasses);
		float yShit = cutY - mainUV.y;
		for (int pass = 0; pass < MAX_BOLD_PASSES; pass++) {
			if (pass >= passCount) break;
			float v = float(pass) / float(passCount);
