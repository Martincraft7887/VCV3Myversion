...
void main()
{
	...
	for (int i = 0; i < MAX_LAYERS; i++) {
		if (i < STARTING_LAYERS || i >= LAYERS) continue;
		float fi = float(i);
		vec2 q = uvCentered*(1.+fi*DEPTH);
		q += vec2(q.y*((WIDTH*(pixely?1.5:1.0))*mod(fi*7.238917,1.)-(WIDTH*(pixely?1.5:1.0))*.5) + ((((SPEED) * (float(LAYERS - i)*.2))*(time*.4))),SPEED*time/(1.+fi*DEPTH*.03));
	...
	vec3 appliedEffect = effect*(pixely?1.6:1.0)*BRIGHT;
	flixelColor.rgb += appliedEffect;
	flixelColor.a = max(flixelColor.a, brightness(appliedEffect));
