...
function loadLEEvents() {
	...
	if (song == null || song.meta == null) return;

	var songName = song.meta.name.toLowerCase();
	var path = 'songs/' + songName + '/LE_events.json';
	if (Assets.exists(Paths.getPath('songs/' + songName + '/LE_events-' + PlayState.difficulty + '.json'))) {
		path = 'songs/' + songName + '/LE_events-' + PlayState.difficulty + '.json';
	}
