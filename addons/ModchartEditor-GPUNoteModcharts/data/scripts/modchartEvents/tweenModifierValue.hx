...
function getItemNameFromXML(node) {
    if (node.exists("property") && node.get("property") != "")
...
function createEventEditor(name, step, item) {
    ...
    return {
        ...
        "step": step,
        "name": data[0],
        "property": data.length > 1 ? data[1] : null,
        "value": 0,
        "time": 4,
        "ease": "cubeInOut",
...
function eventFromXMLEditor(node) {
    ...
    var event = {
        ...
        "step": Std.parseFloat(node.get("step")),
        "name": node.get("name"),
        "property": node.exists("property") ? node.get("property") : null,
