...
function getSubModifierValue(modifier:Dynamic, name:String):Float {
    if (modifier == null || modifier.subMods == null) return 0;
    for (sub in modifier.subMods) {
...
function getNoteModifierVisualOffsetX(strumLineID:Int, strumID:Int, baseCenter:Float):Float {
    if (!noteModchart || modTable == null || modTable.modTable == null) return 0;
    if (modTable.modTable[strumLineID] == null || modTable.modTable[strumLineID][strumID] == null) return 0;

    var x:Float = 0;
    var z:Float = 0;
    var angleZ:Float = 0;

    for (modifier in modTable.modTable[strumLineID][strumID]) {
        if (modifier == null || modifier.shaderFile == null) continue;

        switch(Std.string(modifier.shaderFile).toLowerCase()) {
            case "x":
                x += modifier.value;
            case "z":
                z += modifier.value;
            case "anglez":
                angleZ += modifier.value;
            case "strumlinerotate":
                var rotateZ = getSubModifierValue(modifier, "z");
                if (rotateZ != 0) {
                    var rad = rotateZ * Math.PI / 180;
                    var newPos = 4 + (strumID * ((-4 - 4) / 4));
                    var dist = (112 * newPos * 0.5) - (112 * 0.5);
                    x += dist - (Math.cos(rad) * dist);
    ...
    var center = baseCenter + x;
    var pivot = FlxG.width * 0.5;

    if (angleZ != 0) {
        var rad = angleZ * Math.PI / 180;
        center = pivot + ((center - pivot) * Math.cos(rad));
    }

    if (z != 0) {
        var scale = 1 / Math.max(0.25, 1 - (z * 0.0015));
        center = pivot + ((center - pivot) * scale);
    }
...
function setupItemData(data, node) {
    ...
    data.strumID = Modifier.normalizeStrumIDExpression(node.get("strumID"));
    data.subMods = [];
...
function setupEditMenu(data, itemButton) {
    ...
    var strumIDInput = new UITextBox(16, 166 + 66, data.strumID, 200);
...
function setDataValues(data, itemButton) {
    for (name in ["valueInput", "strumLineIDInput"]) {
    ...
    data.strumID = Modifier.normalizeStrumIDExpression(itemButton.menuObjects.get("strumIDInput").label.text);
    itemButton.menuObjects.get("strumIDInput").label.text = data.strumID;
...
function createNodeFromData(data) {
    ...
    node.set("strumID", Modifier.normalizeStrumIDExpression(data.strumID));
